...
namespace PitLib
{
    class Pseudoflow
    {
        ...
        private void CreateGraphST(Graph g)
        {
            ...
            Vsink = new Vertex(G.V.Count+1, 0);
            //automatizacion del nombre asociado al sink node
            //Console.WriteLine("nr of nodes {0}", G.V.Count+1);
        ...
        public Pseudoflow(Graph g)
        {
            ...
            // modificacion para guardar resultado en el modelo de bloques
            int gap = G.V.Count;
            for (int i = 0; i < NumNodes; ++i)
            {
                int label = AdjacencyList[i].label;
                if (label >= gap)
                {
                    //_bm.BM[label].pit = 1;
                    Console.WriteLine("n {0}", AdjacencyList[i].number);
